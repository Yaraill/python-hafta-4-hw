...
pygame.mixer.music.load("C:/Kodlama/witcher5_demo/UltimatePygameIntro-main/audio/music.wav")
    ...
    if game_active:
        ...
        if current_level in level_up_scores:
            ...
            if score >= next_level_score_target:
                current_level += 1 
                print(f"TEBRİKLER! Seviye {current_level} Oldun!") 
                current_enemy_spawn_interval = max(500, current_enemy_spawn_interval - 200) 
                pygame.time.set_timer(enemy_timer, current_enemy_spawn_interval)
                print(f"Yeni düşman oluşum aralığı: {current_enemy_spawn_interval}ms")
                
                current_fps += 10 
                current_fps = min(current_fps, 100) 
                print(f"Yeni FPS: {current_fps}")
